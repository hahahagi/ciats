    ...
    <div class="bg-white rounded-xl shadow-xl p-8">
        <div class="flex items-center space-x-3 mb-6">
            ...
            <div>
                ...
                <p class="text-gray-600">Perbarui informasi lokasi penyimpanan</p>
            </div>
        </div>

        <form action="{{ route('locations.update', $locationId) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 transition"
                    placeholder="Deskripsi singkat tentang lokasi ini...">{{ old('description', $location['description'] ?? '') }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
